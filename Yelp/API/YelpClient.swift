...
class YelpClient: BDBOAuth1RequestOperationManager {
    ...
    func search(with term: String, completion: @escaping ([Business]?, Error?) -> ()) -> AFHTTPRequestOperation {
        return search(with: term, sort: nil, categories: nil, deals: nil, radius: nil, limit: nil, offset: nil, completion: completion)
    }

    func search(with term: String, sort: YelpSortMode?, categories: [String]?, deals: Bool?, radius: Float?, limit: Int?, offset: Int?, completion: @escaping ([Business]?, Error?) -> ()) -> AFHTTPRequestOperation {
